...
_THINK_ACTION_NAME = "action-think"


def _get_think_action_violation(
    calls: list[object],
    *,
    think_action_enabled: bool,
    send_text_call_name: str,
) -> str | None:
    """校验 think action 是否按规则与 send_text 同轮出现。"""
    if not think_action_enabled:
        return None
    ...
    think_indexes = [idx for idx, name in enumerate(names) if name == _THINK_ACTION_NAME]
    send_indexes = [idx for idx, name in enumerate(names) if name == send_text_call_name]

    if not think_indexes and not send_indexes:
        return None

    if think_indexes and any(not name.startswith("action-") for name in names):
        return (
            "检测到 action-think 与查询型 tool/agent 混用。"
            "action-think 现在只能作为发送回复前的同轮前置 action，"
            "请先完成查询，再在单独一轮里同时调用 action-think 和 action-send_text。"
        )

    if think_indexes and not send_indexes:
        return (
            "检测到单独调用 action-think。"
            "action-think 不允许单独调用，必须与 action-send_text 同轮出现。"
        )

    if send_indexes and not think_indexes:
        return (
            "准备发送回复时，必须在同一轮里先调用 action-think，"
            "然后再调用 action-send_text。"
        )

    if min(think_indexes) > min(send_indexes):
        return (
            "action-think 的调用顺序不正确。"
            "它必须排在 action-send_text 之前。"
        )

    return None


def _append_violation_tool_results(
    response: LLMConversationState,
    calls: list[object],
    violation: str,
) -> None:
    """为整轮违规调用补齐 TOOL_RESULT，驱动模型重试。"""
    for call in calls:
...
def _prepare_runtime_state_user_content(
    response: LLMConversationState,
    payload_content: object,
) -> object:
    """为当前轮 USER payload 准备动态运行时块，并移除旧轮残留。"""
    DefaultChatterPromptBuilder.strip_dynamic_actor_extra_from_payloads(response.payloads)
    dynamic_extra = DefaultChatterPromptBuilder.build_dynamic_actor_extra()
    return DefaultChatterPromptBuilder.append_dynamic_actor_extra(
        payload_content,
        dynamic_extra,
    )
...
async def run_enhanced(
    ...
    while True:
        ...
        if rt.phase == _ToolCallWorkflowPhase.WAIT_USER:
            ...
            unread_user_content = _prepare_runtime_state_user_content(
                rt.response,
                unread_user_content,
            )
        ...
        if rt.phase == _ToolCallWorkflowPhase.TOOL_EXEC:
            ...
            think_violation = _get_think_action_violation(
                llm_response.call_list or [],
                think_action_enabled=usable_map.get(_THINK_ACTION_NAME) is not None,
                send_text_call_name=send_text_call_name,
            )
            if think_violation:
                logger.warning(think_violation)
                _append_violation_tool_results(
                    llm_response,
                    llm_response.call_list or [],
                    think_violation,
                )
                _transition(
                    ...
                    to_phase=_ToolCallWorkflowPhase.FOLLOW_UP,
                    logger=logger,
                    reason="think action contract violated",
                )
                continue
            ...
            # 典型场景：模型同时调用多个 action，再夹带一个查询型 tool，不应继续 follow-up。
...
async def run_classical(
    ...
    while True:
        ...
        if native_multimodal:
            ...
            user_content = _prepare_runtime_state_user_content(request, user_content)
            request.add_payload(LLMPayload(ROLE.USER, user_content))
            history_media_injected = True
        else:
            user_content = _prepare_runtime_state_user_content(
                request,
                Text(classical_user_text),
            )
            request.add_payload(LLMPayload(ROLE.USER, user_content))
