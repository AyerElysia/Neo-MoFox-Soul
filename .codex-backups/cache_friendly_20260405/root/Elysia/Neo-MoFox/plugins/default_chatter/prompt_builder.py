...
class DefaultChatterPromptBuilder:
    ...
    async def build_system_prompt(
        ...
        subconscious_state = store.get(bucket="actor", names=["subconscious"])
        
        # 兜底：如果遭遇跨进程隔离，直接去读文件
        if not subconscious_state:
            import os
            sub_file = "/root/Elysia/Neo-MoFox/data/life_engine_workspace/SUBCONSCIOUS.md"
            if os.path.exists(sub_file):
                try:
                    with open(sub_file, "r", encoding="utf-8") as f:
                        subconscious_state = f.read().strip()
                except Exception:
                    pass
                    
        if subconscious_state:
            subconscious_state = f"# 潜意识状态 (Subconscious State)\n{subconscious_state}"
        ...
        return await (
            ...
            .set("subconscious_state", subconscious_state or "")
            .build()
        )
