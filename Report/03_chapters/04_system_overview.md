...
从《智能感知与无人系统》的学科视角看，Neo-MoFox 解决的是无人系统领域的一个根本性挑战：**如何在无外部触发的情况下维持系统的自主感知与决策连续性**。

传统无人系统（无人车、无人机、服务机器人）普遍采用"感知→规划→控制"的离散循环架构。该架构有两个深层局限：其一，当外部感知输入中断（GPS 遮蔽、网络丢包、传感器盲区），系统的内部状态往往随之冻结，缺乏独立于外部刺激的内驱动力；其二，传感器的感知增益和决策策略通常在离线阶段标定/训练后冻结，无法在部署期真正自适应新环境。

Neo-MoFox 通过三层机制直接回应这两个局限：

1. **持续背景进程（Life Engine 心跳）**：类比无人系统的主控时钟，提供独立于外部感知通道的内驱节奏（30 秒/次），确保即使"感知通道"中断，内部状态仍持续演化。这对应无人系统中"机器人在通信盲区内的自主续航"问题。

2. **分层自适应感知（SNN + 调质 ODE）**：SNN 以秒级 tick 实时处理多维输入事件流，相当于低延迟传感器前端；神经调质 ODE 在分钟-小时尺度对感知增益动态调制，相当于自适应滤波与环境上下文建模。两者构成**多时间尺度的层级感知融合**架构。

3. **离线自我优化（记忆巩固循环）**：在低活跃窗口内完成经验整合与策略修正，类比 SLAM 系统的后端离线优化——系统在"休眠"期间完成本轮地图更新，次日重新激活时已整合了新经验。

这一视角将贯穿本章的架构介绍，也是第 11 章案例研究与第 14 章启示讨论的出发点。
