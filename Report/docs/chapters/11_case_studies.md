...
3. **觉醒后注入**（`service/integrations.py:241`）：梦报告通过 `push_runtime_assistant_injection` 注入 DFC 上下文。
...
4. **觉醒注入**（`service/integrations.py:241`）：梦报告通过 `push_runtime_assistant_injection` 注入 DFC。
...
2. 梦境叙事将桥接路径具象化为连贯故事。
3. 梦报告注入 DFC，LLM 在后续对话中自然提及——**这一"灵感"不是预编程的联想规则，而是记忆图拓扑 + REM 随机游走的涌现结果**。
...
![Figure F14 · 三场景对比（用户离开 5min / 30min / 3hr）](../figures/F14_three_scenarios.svg)

*Figure F14 · 三场景对比（用户离开 5min / 30min / 3hr）*


![Figure F15 · 习惯形成轨迹（streak / strength 双轴）](../figures/F15_habit_trajectory.svg)
