...
| 设计原则 | 智能感知对应 | 无人系统对应 |
|---------|------------|------------|
| **连续性 (C1–C4)** | SNN 的膜电位持续衰减为"感知记忆"留下物理痕迹，使系统能区分 5min 前与 3h 前的刺激强度——这是多传感器时序融合的基础；调质浓度的连续变化等价于感知增益的动态调节 | 系统在外部通信中断期间（类比无人机 GPS 信号丢失）仍维持内部状态演化，保证决策连续性；C4"重启不是重生"直接对应无人系统的**故障容错与状态恢复** |
| **自下而上学习** | 软 STDP 使 SNN 突触自适应标定：哪些输入特征对哪个驱动最有预测力，由真实交互数据在线决定，等价于传感器的**自适应增益校准**，无需离线参数整定 | 奖赏调制 + Hebbian 规则使无人系统的行为策略**持续演化**而无需重新训练；习惯强度机制让智能体在长期部署中自发涌现出稳定的行为模式，对应强化学习中的在线策略优化 |
| **系统涌现智能** | 单个 SNN 神经元仅响应简单特征；五调质的协作使系统具备情绪-时间-社交多维度的**语义级感知**，这种多维感知不是任何单一模块能实现的，必须来自异质子系统的涌现 | LLM 的强推理 + SNN 的快速在线学习 + 调质的慢速情态调控，三者共同构成**感知-决策-执行**的完整闭环，缺少任意一层系统整体能力都会显著退化——这正是无人系统"不能靠单一算法包打天下"的工程实践 |
